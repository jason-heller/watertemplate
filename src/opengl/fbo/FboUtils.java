...
public class FboUtils {
	...
	private static final int fboSamplingAmt = 2;
	...
	public static int createDepthBufferAttachment(int width, int height, boolean multisampled) {
		...
		if (multisampled) {
			GL30.glRenderbufferStorageMultisample(GL30.GL_RENDERBUFFER, fboSamplingAmt, GL11.GL_DEPTH_COMPONENT, width,
					height);
		} else {
	...
	public static int createMultisampleColorAttachment(int width, int height, int attachment) {
		...
		GL30.glRenderbufferStorageMultisample(GL30.GL_RENDERBUFFER, fboSamplingAmt, GL11.GL_RGBA8, width, height);
	...
	public static void resolve(FrameBuffer input) {
		...
		GL30.glBlitFramebuffer(0, 0, input.getWidth(), input.getHeight(), 0, 0, Display.getWidth(), Display.getHeight(),
